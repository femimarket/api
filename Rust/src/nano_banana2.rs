...
/// POSTs a NanoBanana2 Model to femi.market with HTTP Basic auth (`user:password`).
...
pub extern "C" fn rust_ffi_nano_banana2(
    ...
    let body = serde_json::json!({
        "id": uuid::Uuid::now_v7(),
        "user_id": "",
        "action": { "type": "NanoBanana2", "prompt": s(prompt).unwrap_or_default(), "fal_request_id": "", "file": "" },
        "status": "Pending",
        "credit": 0,
    });
