...
pub(crate) async fn core_flux2_klein_i2i(user: String, password: String, image_b64: String, image2_b64: String, prompt: String) -> Vec<u8> {
...
pub mod native {
    ...
    #[unsafe(no_mangle)]
    pub extern "C" fn rust_ffi_flux2_klein_i2i(
        user: *const c_char,
        password: *const c_char,
        image_b64: *const c_char,
        image2_b64: *const c_char,
        prompt: *const c_char,
        ...
        let i1 = s(image_b64).unwrap_or_default();
        let i2 = s(image2_b64).unwrap_or_default();
        ...
        let bytes = rt().block_on(async move {
            let do_call = core_flux2_klein_i2i(u, pw, i1, i2, pr);
            if cancel_addr == 0 {
                do_call.await
            } else {
...
pub mod wasm {
    ...
    #[wasm_bindgen]
    pub async fn wasm_flux2_klein_i2i(user: String, password: String, image_b64: String, image2_b64: String, prompt: String) -> js_sys::Uint8Array {
        let bytes = core_flux2_klein_i2i(user, password, image_b64, image2_b64, prompt).await;
