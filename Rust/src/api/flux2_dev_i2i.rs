...
pub(crate) async fn core_flux2_dev_i2i(user: String, password: String, image_b64: String, prompt: String) -> Vec<u8> {
...
pub mod native {
    ...
    pub extern "C" fn rust_ffi_flux2_dev_i2i(
        ...
        let bytes = rt().block_on(async move {
            let do_call = core_flux2_dev_i2i(u, pw, i, pr);
            if cancel_addr == 0 {
                do_call.await
            } else {
...
pub mod wasm {
    ...
    #[wasm_bindgen]
    pub async fn wasm_flux2_dev_i2i(user: String, password: String, image_b64: String, prompt: String) -> js_sys::Uint8Array {
        let bytes = core_flux2_dev_i2i(user, password, image_b64, prompt).await;
