...
pub(crate) async fn core_nano_banana2(user: String, password: String, prompt: String) -> Vec<u8> {
    let body = serde_json::json!({
        "id": uuid::Uuid::now_v7(),
        "user_id": "",
        "action": { "type": "NanoBanana2", "prompt": prompt, "fal_request_id": "", "file": "" },
    });
...
pub mod native {
    ...
    #[unsafe(no_mangle)]
    pub extern "C" fn rust_ffi_nano_banana2(
        user: *const c_char,
        ...
        let bytes = rt().block_on(async move {
            let do_call = core_nano_banana2(u, pw, pr);
            if cancel_addr == 0 {
                do_call.await
            } else {
...
pub mod wasm {
    ...
    #[wasm_bindgen]
    pub async fn wasm_nano_banana2(user: String, password: String, prompt: String) -> js_sys::Uint8Array {
        let bytes = core_nano_banana2(user, password, prompt).await;
