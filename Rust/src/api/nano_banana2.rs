...
pub(crate) async fn core_nano_banana2(prompt: String) -> Vec<u8> {
...
pub mod native {
    ...
    pub extern "C" fn rust_ffi_nano_banana2(
        ...
        let bytes = rt().block_on(async move {
            let do_call = core_nano_banana2(pr);
            if cancel_addr == 0 {
                do_call.await
            } else {
...
pub mod wasm {
    ...
    #[wasm_bindgen]
    pub async fn wasm_nano_banana2(prompt: String) -> js_sys::Uint8Array {
        let bytes = core_nano_banana2(prompt).await;
