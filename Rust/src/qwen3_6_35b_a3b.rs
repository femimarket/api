...
/// POSTs a Qwen3_6_35bA3b Model to femi.market with HTTP Basic auth (`user:password`).
...
pub extern "C" fn rust_ffi_qwen3_6_35b_a3b(
    ...
    let messages: serde_json::Value = s(messages_json)
        .and_then(|j| serde_json::from_str(&j).ok())
        .unwrap_or_else(|| serde_json::Value::Array(Vec::new()));
    let body = serde_json::json!({
        "id": uuid::Uuid::now_v7(),
        "user_id": "",
        "action": { "type": "Qwen3_6_35bA3b", "messages": messages },
        "status": "Pending",
        "credit": 0,
    });
